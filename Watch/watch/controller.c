...
{
    printf("handle_av_events: lparam %04x   state: %04x rparam  %s\n",lparam,state,rparam);
	switch(lparam)
    {
...
{
	printf("control_process: ev %04x   laparam %04x state: %04x rparam  %s\n",ev,lparam,state,rparam);
  ...
  case EVENT_WINDOW_ACTIVE:
    {
      ...
      {
        // gesture is enabled
        //gesture_init(0, flag & BIT1); hawpaw: dissabled
      }
