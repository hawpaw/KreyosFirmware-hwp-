  ...
  switch(ev)
  {
    ...
    case EVENT_WINDOW_PAINT:
    {
	  ...
	  if (state == 5)
	  {
	  	...
	  	GrStringDrawWrap(pContext, "Battery charged.", 10, 90, LCD_WIDTH - 20, ALIGN_CENTER);
	  }
	  else if (state == 6)
	  {
	  	...
	  	GrContextFontSet(pContext, (tFont*)&g_sFontGothic24b);
	  	GrStringDrawWrap(pContext, "Low battery\nPlug the charger.", 10, 90, LCD_WIDTH - 20, ALIGN_CENTER);
	  }
	  else
	  {
		...
		GrContextFontSet(pContext, (tFont*)&g_sFontGothic24b);
	  	GrStringDrawWrap(pContext, "Charging battery.", 10, 90, LCD_WIDTH - 20, ALIGN_CENTER);
	  }
      break;
	}
