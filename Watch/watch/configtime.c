...
//hawpaw addition
static void OnDrawData_EUUS_Format(tContext *pContext, int config)
{
    // clear the region
  GrContextForegroundSet(pContext, ClrBlack);
  GrRectFill(pContext, &client_clip);
  ...
  GrContextFontSet(pContext, &g_sFontGothic24b);
  int width = GrStringWidthGet(pContext, date_format_short[config], -1);
  window_selecttext(pContext, date_format_short[config], -1, 72 - width/2, 70);

  window_button(pContext, KEY_ENTER, "OK");
}

uint8_t date_format_process(uint8_t event, uint16_t lparam, void* rparam)
{
	switch(event)
  {
  case EVENT_WINDOW_CREATED:
    {
      state = window_readconfig()->date_format;
      break;
    }
  case EVENT_WINDOW_PAINT:
    {
    	OnDrawData_EUUS_Format((tContext*)rparam, state);
    	 ...
    	 switch(lparam)
    	      {
    	        ...
    	        break;
    	        case KEY_ENTER:
    	          window_readconfig()->date_format = state;
    	          window_writeconfig();
    	          window_close();
    	          return 1;
    	      }
