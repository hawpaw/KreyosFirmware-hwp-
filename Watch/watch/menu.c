...
#define DATA_FORMAT_SHORT 0xE0
#define MOTION_BACKLIGHT   0xE1
...
static const struct MenuItem SetupMenu[] =
{
  ...
  {DATA_FORMAT_SHORT, "Date format", &date_format_process},
 // {MOTION_BACKLIGHT,"M. Backlight", &motion_backlight},
...
static void drawMenuItem(tContext *pContext, const tFont* textFont, int MENU_SPACE, const struct MenuItem *item, int index, int selected)
{
  ...
  else
  {
    ...
    switch(item->icon)
    {
      ...
      case DATA_FORMAT_SHORT:
            {
              strcpy(buf, date_format_short[window_readconfig()->date_format]);
              break;
            }
